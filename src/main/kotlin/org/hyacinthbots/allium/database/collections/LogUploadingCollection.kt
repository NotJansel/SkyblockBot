...
class LogUploadingCollection : KordExKoinComponent {
	...
	suspend fun addChannelToWhitelist(guildId: Snowflake, channelId: Snowflake) {
		...
		if (list != null) {
			list.add(channelId)
			whitelistCollection.updateOne(
				LogUploadingWhitelistData::guildId eq guildId,
				setValue(LogUploadingWhitelistData::channels, list)
			)
		} else {
	...
	suspend fun addChannelToBlacklist(guildId: Snowflake, channelId: Snowflake) {
		...
		if (list != null) {
			list.add(channelId)
			blacklistCollection.updateOne(
				LogUploadingBlacklistData::guildId eq guildId,
				setValue(LogUploadingBlacklistData::channels, list)
			)
		} else {
			list = mutableListOf(channelId)
	...
	suspend fun getBlacklist(guildId: Snowflake): MutableList<Snowflake>? =
		if (blacklistCollection.findOne(LogUploadingBlacklistData::guildId eq guildId) != null) {
			blacklistCollection.findOne(LogUploadingBlacklistData::guildId eq guildId)!!.channels
		} else {
			null
		}
	...
	suspend fun removeWhitelist(guildId: Snowflake) =
		whitelistCollection.deleteOne(LogUploadingWhitelistData::guildId eq guildId)
